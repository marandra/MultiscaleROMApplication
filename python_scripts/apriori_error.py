...
def make_list_of_files(conf, traj):
    ...
    for path in trajectory_paths:
        ...
        ene_e_files.extend(
            sorted(glob.glob("{}/{}*".format(path, energy_filename)))[:nr_e_snap]
        )
        ene_i_files.extend(
            sorted(glob.glob("{}/{}*".format(path, energy_filename)))[nr_e_snap:]
        )
        str_e_files.extend(
            sorted(glob.glob("{}/{}*".format(path, strain_filename)))[:nr_e_snap]
        )
        str_i_files.extend(
            sorted(glob.glob("{}/{}*".format(path, strain_filename)))[nr_e_snap:]
        )
...
group.add_argument(
    "-e", "--only-energy", action="store_true", help="compute only energy reduced bases"
)
group.add_argument(
    "-s", "--only-strain", action="store_true", help="compute only strain reduced bases"
)
