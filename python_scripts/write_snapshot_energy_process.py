...
def Factory(settings, model):
    return WriteSnapshotEnergy(settings["Parameters"], model)


class WriteSnapshotEnergy(Kratos.Process):
    ...
    def write_results(self, filename):
        ...
        for elem in self.model_part.Elements:
            strain_energy_values = elem.GetValuesOnIntegrationPoints(
                Kratos.STRAIN_ENERGY, self.model_part.ProcessInfo)
            for strain_energy_ip in strain_energy_values:
                write_value(ofile, strain_energy_ip[0])
