...
def create_properties_file(props_fname, training_path, traj_name, case_path):
    ...
    training_props_path = PurePath.joinpath(training_path, Path("trajectory_{}".format(traj_name[1:][:-1])), props_path)
    with training_props_path.open() as fi:
        props = json.load(fi)
        strain_versor = props["processes"]["loads_process_list"][0]["Parameters"]["initial_strain"]
        ampl = props["processes"]["loads_process_list"][0]["Parameters"]["lookuptable_mult"][-1]
    with props_path.open() as fi:
        props = json.load(fi)
        # compute displacements u = E * x
        s0, s1, s2, s3, s4, s5 = strain_versor
        x0 = 1.0 * s0 * ampl; y0 = 0.5 * s3 * ampl; z0 = 0.5 * s5 * ampl;
        x1 = 0.5 * s3 * ampl; y1 = 1.0 * s1 * ampl; z1 = 0.5 * s4 * ampl;
        x2 = 0.5 * s5 * ampl; y2 = 0.5 * s4 * ampl; z2 = 1.0 * s2 * ampl;
        props["processes"]["list_boundary_processes"][1]["Parameters"]["value"] = ["{}*t".format(x0), "{}*t".format(x1), "{}*t".format(x2)] 
        props["processes"]["list_boundary_processes"][2]["Parameters"]["value"] = ["{}*t".format(y0), "{}*t".format(y1), "{}*t".format(y2)] 
        props["processes"]["list_boundary_processes"][3]["Parameters"]["value"] = ["{}*t".format(z0), "{}*t".format(z1), "{}*t".format(z2)] 
...
def create_materials_file(materials_fname, offline_path, rve_name, case_path):
    ...
    with materials_path.open() as fi:
        ...
        materials["properties"][0]["Material"]["constitutive_law"]["Parameters"]["rve_data_filename"] = str(rve_data_path)
...
def create_case_dir(traj_name, rve_name, training_path, offline_path):
    ...
    create_properties_file("ProjectParameters.json", training_path, traj_name, case_path)
...
if __name__ == "__main__":
    ...
    modes = [10, 20, 30]
    points = [50, 100, 200, 400, 800, 1600, 3200, "ROM"]
    points = [50, 200, "ROM"]
    ...
    for t in trajectories:
        for m in modes:
            for p in points:
                ...
                create_case_dir(traj_name, rve_name, training_path, offline_path)
