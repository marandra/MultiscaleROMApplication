...
#pragma omp parallel for firstprivate(nconditions, RHS_Contribution, EquationId)
    ...
    )
    {
		...
		if (this->GetEchoLevel() > 1 && r_model_part.GetCommunicator().MyPID() == 0)
		{
            ...
            //ModelPart::DofsArrayType DofSet = r_model_part.GetNode(1).GetDofs();
            //ModelPart::DofsArrayType DofSet = r_model_part.GetNode(1).GetDofs();
            BaseType::mDofSet = DofsArrayType();
            //Dof<double> & mydof = r_model_part.GetNode(0).GetDof(DISPLACEMENT_X);
            //BaseType::mDofSet[0] = mydof;
/*
...
*/
    ...
    void Clear()
    {
        ...
        if (this->GetEchoLevel() > 1)
        {
            std::cout << "ROMBuilderAndSolver Clear Function called" << std::endl;
        }
    }
