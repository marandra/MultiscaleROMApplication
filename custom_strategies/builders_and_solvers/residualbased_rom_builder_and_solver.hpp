...
class ResidualBasedROMBuilderAndSolver
    : public ResidualBasedEliminationBuilderAndSolver<TSparseSpace, TDenseSpace,
                                                      TLinearSolver> {
public:
  ...
  ResidualBasedROMBuilderAndSolver(
      ...
      : ResidualBasedEliminationBuilderAndSolver<TSparseSpace, TDenseSpace,
                                                 TLinearSolver>(
  ...
  void Build(typename TSchemeType::Pointer pScheme, ModelPart &r_model_part,
             TSystemMatrixType &A, TSystemVectorType &b) {
    KRATOS_TRY
    ...
    for (int k = 0; k < nelements; k++) {
      ...
      pScheme->CalculateSystemContributions(*(it.base()), LHS_Contribution,
  ...
  void ResizeAndInitializeVectors(TSystemMatrixPointerType &pA,
    ...
    if (pA == NULL) {
      TSystemMatrixPointerType pNewA =
          TSystemMatrixPointerType(new TSystemMatrixType(0, 0));
      pA.swap(pNewA);
    }
    if (pDx == NULL) {
      TSystemVectorPointerType pNewDx =
          TSystemVectorPointerType(new TSystemVectorType(0));
      pDx.swap(pNewDx);
    }
    if (pb == NULL) {
      TSystemVectorPointerType pNewb =
          TSystemVectorPointerType(new TSystemVectorType(0));
      pb.swap(pNewb);
    }
    ...
    if (BaseType::mCalculateReactionsFlag == true) {
      unsigned int ReactionsVectorSize =
          BaseType::mDofSet.size() - BaseType::mEquationSystemSize;
...
#pragma omp parallel for firstprivate(nelements, ids)
    for (int iii = 0; iii < nelements; iii++) {
      typename ElementsContainerType::iterator i_element =
          rElements.begin() + iii;
...
#pragma omp parallel for firstprivate(nconditions, ids)
    for (int iii = 0; iii < nconditions; iii++) {
      typename ConditionsArrayType::iterator i_condition =
          rConditions.begin() + iii;
    ...
    A = boost::numeric::ublas::compressed_matrix<double>(indices.size(),
                                                         indices.size(), nnz);
