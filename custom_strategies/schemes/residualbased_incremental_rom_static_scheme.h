...
class ResidualBasedIncrementalROMStaticScheme : public ResidualBasedIncrementalUpdateStaticScheme<TSparseSpace,TDenseSpace>
{
public:
    ...
    virtual void Initialize(ModelPart& r_model_part){
        ...
        std::size_t nr_modes = r_model_part.GetProcessInfo()[REDUCED_MODES_WEIGHTS].size();
    ...
    virtual void Update(ModelPart &r_model_part, DofsArrayType &rDofSet,
                        TSystemMatrixType &A, TSystemVectorType &Dx, TSystemVectorType &b) {
        TSystemVectorType & modes_weights = r_model_part.GetProcessInfo().GetValue(REDUCED_MODES_WEIGHTS);
