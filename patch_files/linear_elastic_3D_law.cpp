...
bool LinearElastic3DLaw::Has(const Variable<Vector>& rThisVariable)
{
...
{
	Kratos::HyperElastic3DLaw::GetValue(rThisVariable, rValue);
...
Vector& LinearElastic3DLaw::GetValue(const Variable<Vector>& rThisVariable, Vector& rValue)
{
...
Matrix& LinearElastic3DLaw::GetValue(const Variable<Matrix>& rThisVariable, Matrix& rValue)
{
	return rValue;
}

void LinearElastic3DLaw::SetValue(const Variable<Vector >& rVariable,
...
void LinearElastic3DLaw::InitializeMaterial(
	const Properties& rMaterialProperties,
	const GeometryType& rElementGeometry,
	const Vector& rShapeFunctionsValues)
{
	Kratos::HyperElastic3DLaw::InitializeMaterial(rMaterialProperties, rElementGeometry, rShapeFunctionsValues);
	m_init_strain = ZeroVector(this->GetStrainSize());
}

void LinearElastic3DLaw::ResetMaterial(const Properties& rMaterialProperties,
	const GeometryType& rElementGeometry,
	const Vector& rShapeFunctionsValues)
{
}
					  ...
					  Vector& rStressVector )
{
    ...
    noalias(rStressVector) = prod(rConstitutiveMatrix,rStrainVector); // - m_init_strain);
}
