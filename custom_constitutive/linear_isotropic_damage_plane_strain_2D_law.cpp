...
namespace Kratos
{
	...
	double d, q;
	    ...
	    // C_tan = (1 - d) * C
            if (tau_e <= r_prev) {
                r = r_prev;
                q = CalculateQLinear(r, matprops);
                //q = CalculateQExponential(r, matprops);
                d = 1. - q / r;
                stress *= (1 - d); 
    ...
    double LinearIsotropicDamagePlaneStrain2DLaw::CalculateQExponential(double r,
    	const Properties& material_prop) {
    ...
    double LinearIsotropicDamagePlaneStrain2DLaw::CalculateQLinear(double r,
    	const Properties& material_prop) {
