...
namespace Kratos
{
    ...
    double& SmallDisplacementIsotropicDamage3DLaw::GetValue(const Variable<double>& rThisVariable, double& rValue)
    {
    	return rValue;
    }
    
    Vector& SmallDisplacementIsotropicDamage3DLaw::GetValue(const Variable<Vector>& rThisVariable, Vector& rValue)
    {
    	return rValue;
    }
    
    Matrix& SmallDisplacementIsotropicDamage3DLaw::GetValue(const Variable<Matrix>& rThisVariable, Matrix& rValue)
    {
    	return rValue;
    }
    
    array_1d<double, 2 > & SmallDisplacementIsotropicDamage3DLaw::GetValue(const Variable<array_1d<double, 2 > >& rVariable, array_1d<double, 2 > & rValue)
    {
    	return rValue;
    }
    
    array_1d<double, 3 > & SmallDisplacementIsotropicDamage3DLaw::GetValue(const Variable<array_1d<double, 3 > >& rVariable, array_1d<double, 3 > & rValue)
    {
    	return rValue;
    }
    ...
    void SmallDisplacementIsotropicDamage3DLaw::CalculateMaterialResponseCauchy(Parameters& rValues)
    {
		...
		if (TRACTION_ONLY){
            ...
            */
            ...
            KRATOS_THROW_ERROR(std::invalid_argument,"Isotropic 3D Damage - Yield surface not already implemented","");
    	...
    	const ProcessInfo& rCurrentProcessInfo)
    {
    		...
    		KRATOS_THROW_ERROR(std::invalid_argument, "LinearIsotropicDamagePlaneStrain2DLaw - missing ISOTROPIC_DAMAGE_MODULUS", "");
    		...
    		KRATOS_THROW_ERROR(std::invalid_argument, "LinearIsotropicDamagePlaneStrain2DLaw - ISOTROPIC_DAMAGE_MODULUS must be < 1.", "");
    		return 0;
    }
