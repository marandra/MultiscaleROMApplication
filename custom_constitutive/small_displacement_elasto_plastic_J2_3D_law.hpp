...
namespace Kratos {
class KRATOS_API(MULTISCALE_ROM_APPLICATION)
    SmallDisplacementElastoPlasticJ23DLaw : public ConstitutiveLaw {
public:
  ...
  virtual Matrix &GetValue(const Variable<Matrix> &rThisVariable,
                           Matrix &rValue);
  virtual array_1d<double, 2> &
  GetValue(const Variable<array_1d<double, 2>> &rVariable,
           array_1d<double, 2> &rValue);
  virtual array_1d<double, 3> &
  GetValue(const Variable<array_1d<double, 3>> &rVariable,
           array_1d<double, 3> &rValue);
  virtual void SetValue(const Variable<double> &rVariable, const double &rValue,
                        const ProcessInfo &rCurrentProcessInfo);
  virtual void SetValue(const Variable<Vector> &rVariable, const Vector &rValue,
                        const ProcessInfo &rCurrentProcessInfo);
  virtual void SetValue(const Variable<Matrix> &rVariable, const Matrix &rValue,
                        const ProcessInfo &rCurrentProcessInfo);
  ...
  virtual void FinalizeSolutionStep(const Properties &rMaterialProperties,
                                    ...
                                    const ProcessInfo &rCurrentProcessInfo);
  virtual void
  InitializeNonLinearIteration(const Properties &rMaterialProperties,
                               const GeometryType &rElementGeometry,
                               const Vector &rShapeFunctionsValues,
                               const ProcessInfo &rCurrentProcessInfo);
  virtual void
  FinalizeNonLinearIteration(const Properties &rMaterialProperties,
                             const GeometryType &rElementGeometry,
                             const Vector &rShapeFunctionsValues,
                             const ProcessInfo &rCurrentProcessInfo);
...
protected:
  ...
  double GetDeltaGamma(double norm_s_trial,
                       const Properties &rMaterialProperties);
  virtual void CalculateTangentTensor(double dgamma, double norm_s_trial,
                                      const Vector &N_new,
                                      const Properties &props, Matrix &D);
  virtual void CalculateElasticityTensor(const Properties &props,
