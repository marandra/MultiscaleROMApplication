...
RVELaw::RVELaw(Kratos::Parameters Params) {
  ...
  mRelativeTolerance =
      Params["Parameters"]["residual_relative_tolerance"].GetDouble();
  mAbsoluteTolerance =
      Params["Parameters"]["residual_absolute_tolerance"].GetDouble();
  ...
  const double angle =
      Params["Parameters"]["rotation"]["angle_degree"].GetDouble() *
      3.141592653589793 / 180;
  mQ = Quaternion<double>::FromAxisAngle(ax, ay, az, angle);
  mQ.normalize();
  KRATOS_INFO("RVELaw") << "RVE rotation: " << angle << " rad on [" << ax
                        << ", " << ay << ", " << az << "]"
                        << " (q = " << mQ.W() << " + " << mQ.X() << "i + "
                        << mQ.Y() << "j + " << mQ.Z() << "k)" << std::endl;
      ...
      for (std::size_t mm = 0; mm < modified_material.size(); ++mm) {
        ...
        if (op == mp) {
          properties.GetArrayItem(om)["Material"].SetValue(
              "Variables",
              modified_material.GetArrayItem(mm)["Material"]["Variables"]);
  ...
  std::string mp_name =
      properties.GetArrayItem(0)["model_part_name"].GetString();
  LSplit(mp_name);
  ...
  for (std::size_t om = 0; om < properties.size(); ++om) {
    std::string mp_name =
        properties.GetArrayItem(om)["model_part_name"].GetString();
    RSplit(mp_name);
  ...
  for (std::size_t om = 0; om < properties.size(); ++om) {
    const std::string mp_name =
        properties.GetArrayItem(om)["model_part_name"].GetString();
    const std::size_t property_id =
        properties.GetArrayItem(om)["properties_id"].GetInt();
  ...
  for (std::size_t i = 0; i < nr_points; i++) {
    ...
    const ConstitutiveLaw::Pointer pcl =
        prop.GetValue(CONSTITUTIVE_LAW)->Clone();
...
RVELaw::RVELaw(PropertiesMap pProperties, std::vector<Matrix> B_list,
               ...
               std::vector<int> prop_id_list, double abs_tol, double rel_tol,
               int max_iter, int verbose, Quaternion<double> q_rotation)
    : mProperties_map(pProperties), mB_vec(B_list), mIW_vec(IW_list),
      mCL_vec(CL_list), mPropId_vec(prop_id_list), mAbsoluteTolerance(abs_tol),
      mRelativeTolerance(rel_tol), mMaxIteration(max_iter), mVerbose(verbose),
      mQ(q_rotation) {
...
void RVELaw::FinalizeMaterialResponseCauchy(
    ConstitutiveLaw::Parameters &rParametersValues) {
  FinalizeMaterialResponsePK2(rParametersValues);
}

void RVELaw::FinalizeMaterialResponsePK1(
    ConstitutiveLaw::Parameters &rParametersValues) {
  FinalizeMaterialResponsePK2(rParametersValues);
}

void RVELaw::FinalizeMaterialResponseKirchhoff(
    ConstitutiveLaw::Parameters &rParametersValues) {
  FinalizeMaterialResponsePK2(rParametersValues);
}
...
void RVELaw::CalculateMaterialResponseCauchy(
  ...
  // while (residual > mAbsoluteTolerance and ratio > mRelativeTolerance and it
  // < mMaxIteration)
  //{
  //    Solve(A, res, Dx);
  //    mModesWeights -= Dx;
...
void RVELaw::CalculateMaterialResponsePK1(
    ConstitutiveLaw::Parameters &rParametersValues) {
  CalculateMaterialResponseCauchy(rParametersValues);
}
void RVELaw::CalculateMaterialResponsePK2(
    ConstitutiveLaw::Parameters &rParametersValues) {
  CalculateMaterialResponseCauchy(rParametersValues);
}
void RVELaw::CalculateMaterialResponseKirchhoff(
    ConstitutiveLaw::Parameters &rParametersValues) {
  CalculateMaterialResponseCauchy(rParametersValues);
}
...
//            cl_params.SetProcessInfo(process_info);
//
//            // IS_INELASTIC is retrieved with CalculateValue, so Has() returns
//            "false" if (not mCL_vec[i]->Has(IS_INELASTIC)) {
...
double &RVELaw::CalculateValue(ConstitutiveLaw::Parameters &rParametersValues,
  ...
  if (rThisVariable == STRAIN_ENERGY) {
    ...
    CalculateValue(rParametersValues, STRAIN_ENERGY_VECTOR,
                   r_strain_energy_vector);
    rValue = 0.0;
    for (std::size_t i = 0; i < nr_points; i++) {
...
std::string RVELaw::ReadFile(const std::string &filename) const {
  ...
  KRATOS_ERROR_IF_NOT(infile.good())
      << "File " << filename << " cannot be found" << std::endl;
...
int RVELaw::Check(const Properties &rUnusedProperties,
  ...
  for (std::size_t i = 0; i < nr_points; i++) {
    const Properties material_props = mProperties_map[mPropId_vec[i]];
    const GeometryType dummy_element_geometry;
    mCL_vec[i]->Check(material_props, dummy_element_geometry,
                      rCurrentProcessInfo);
  }
