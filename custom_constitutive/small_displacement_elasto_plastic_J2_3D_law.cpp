...
namespace Kratos
{
    ...
    void SmallDisplacementElastoPlasticJ23DLaw::CalculateMaterialResponseCauchy(Parameters& rValues)
    {
        ...
        double norm_dev_stress = std::sqrt(StressTrialDev(0) * StressTrialDev(0) + StressTrialDev(1) * StressTrialDev(1)
                                           ...
                                           + 2. * StressTrialDev(4) * StressTrialDev(4)+ 2. * StressTrialDev(5) * StressTrialDev(5));
        ...
        if (trial_yield_function <= 0.){
            ...
            StressVector(4) = StressTrialDev(4) - 2. * mu * dgamma * YieldFunctionNormalVector(4);
            StressVector(5) = StressTrialDev(5) - 2. * mu * dgamma * YieldFunctionNormalVector(5);
