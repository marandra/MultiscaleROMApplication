...
def compute_energy_elastic(conf, energy_elastic_files):
    ...
    nr_elements = int(conf['Parameters']['nr_elements'])
    nr_integration_points = int(conf['Parameters']['nr_integration_points'])
    tol_svd_elastic_energy = float(conf['Parameters']['tolerance_svd_elastic_energy'])
    ...
    for iValue, sin_val in enumerate(S):
        if sin_val > tol_svd_elastic_energy:
            if cont == 1:
                Ue_el = U[:, iValue]
            else:
...
def compute_energy_inelastic(conf, energy_inelast_files, Ue_el):
    ...
    nr_elements = int(conf['Parameters']['nr_elements'])
    nr_integration_points = int(conf['Parameters']['nr_integration_points'])
    tol_svd_inelastic_energy = float(conf['Parameters']['tolerance_svd_inelastic_energy'])
    ...
    Ue_in = []
    for iValue, sin_val in enumerate(S):
        if sin_val > tol_svd_inelastic_energy:
            if cont == 1:
                Ue_in = U[:, iValue]
            else:
...
def computed_complete_set():
    ...
    nr_dofs = p["nr_elements"] * nr_integration_points * nr_strain_components
    X = np.empty([nr_dofs, len(strain_elastic_files)])
    for i, file in enumerate(strain_elastic_files):
        X[:, i] = np.fromfile(file, dtype=np.float32)
    # TODO: incluir el SVD del paquete scipy, parece que es mas optimo, ver si esta instalada una version reciente de scipy en el cluster
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2 or '-h' in sys.argv:
        sys.exit("\nusage: python3 {} [-h|--help] config_file\n".format(sys.argv[0]) +
            "description: computes energy and strain reduced bases.\n" )
    ...
    Ue = np.hstack([Ue_el, Ue_in])
    # TODO: can we save the whole matrix?
    with open(energy_bases_filename, 'wb') as ofile:
