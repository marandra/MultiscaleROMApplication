...
SmallDisplacementBbarElement::SmallDisplacementBbarElement() : Element()
{
...
{
    return Element::Pointer(new SmallDisplacementBbarElement(
...
{
    SmallDisplacementBbarElement NewElement(
...
void SmallDisplacementBbarElement::GetDofList(DofsVectorType& rElementalDofList,
...
void SmallDisplacementBbarElement::EquationIdVector(EquationIdVectorType& rResult,
                                                    ProcessInfo& rCurrentProcessInfo)
{
...
void SmallDisplacementBbarElement::SetValueOnIntegrationPoints(
...
void SmallDisplacementBbarElement::GetValueOnIntegrationPoints(
    const Variable<ConstitutiveLaw::Pointer>& rVariable,
    std::vector<ConstitutiveLaw::Pointer>& rValues,
    const ProcessInfo& rCurrentProcessInfo)
{
                                                              ...
                                                              const ProcessInfo& rCurrentProcessInfo)
{
    ...
    unsigned int voigt_size =
        4; // added component zz, necessary for plasticity.
...

{
    ...
    if (rCalculationFlags.Is(SmallDisplacementBbarElement::COMPUTE_LHS_MATRIX)) // calculation of the matrix is required
    {
        ...
        noalias(rLeftHandSideMatrix) =
            ZeroMatrix(MatSize, MatSize); // resetting LHS
    }
                                                            ...
                                                            ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int PointNumber = 0; PointNumber < integration_points.size(); PointNumber++)
    {
        ...
        this->CalculateKinematics(Variables,
                                  PointNumber); // JLM ver como pasar rBh
        // if(PointNumber ==
        // 1){KRATOS_WATCH(Variables.B);KRATOS_WATCH(Variables.Bh);}
        ...
        // if ( dimension == 2 ) IntegrationWeight *=
                                                          ...
                                                          ProcessInfo& rCurrentProcessInfo)
{
    ...
    this->InitializeSystemMatrices(LocalLeftHandSideMatrix, LocalRightHandSideVector,
...
void SmallDisplacementBbarElement::CalculateAndAddLHS(LocalSystemComponents& rLocalSystem,
...
void SmallDisplacementBbarElement::CalculateAndAddRHS(LocalSystemComponents& rLocalSystem,
                                                      GeneralVariables& rVariables,
                                                      Vector& rVolumeForce,
                                                      double& rIntegrationWeight)
{
    ...
    if (rLocalSystem.CalculationFlags.Is(SmallDisplacementBbarElement::COMPUTE_RHS_VECTOR_WITH_COMPONENTS))
    {
        ...
        for (unsigned int i = 0; i < rRightHandSideVariables.size(); i++)
        {
            ...
            if (rRightHandSideVariables[i] == EXTERNAL_FORCES_VECTOR)
            {
                ...
                this->CalculateAndAddExternalForces(rRightHandSideVectors[i], rVariables,
...
void SmallDisplacementBbarElement::CalculateAndAddDynamicLHS(MatrixType& rLeftHandSideMatrix,
                                                             GeneralVariables& rVariables,
                                                             ProcessInfo& rCurrentProcessInfo,
                                                             double& rIntegrationWeight)
{
        ...
        for (unsigned int k = 0; k < dimension; k++)
        {
            ...
            for (unsigned int j = 0; j < number_of_nodes; j++)
            {
                rLeftHandSideMatrix(indexi + k, indexj + k) +=
                    rVariables.N[i] * rVariables.N[j] * CurrentDensity * rIntegrationWeight;
                indexj += dimension;
            }
        }
...
void SmallDisplacementBbarElement::CalculateAndAddDynamicRHS(VectorType& rRightHandSideVector,
                                                             GeneralVariables& rVariables,
                                                             ProcessInfo& rCurrentProcessInfo,
                                                             double& rIntegrationWeight)
{
...
void SmallDisplacementBbarElement::CalculateRightHandSide(VectorType& rRightHandSideVector,
                                                          ProcessInfo& rCurrentProcessInfo)
{
...
void SmallDisplacementBbarElement::CalculateRightHandSide(
    std::vector<VectorType>& rRightHandSideVectors,
...
void SmallDisplacementBbarElement::CalculateLeftHandSide(MatrixType& rLeftHandSideMatrix,
                                                         ProcessInfo& rCurrentProcessInfo)
{
    // create local system components
    LocalSystemComponents LocalSystem;
...
void SmallDisplacementBbarElement::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix,
                                                        VectorType& rRightHandSideVector,
                                                        ProcessInfo& rCurrentProcessInfo)
{
    // create local system components
...
void SmallDisplacementBbarElement::CalculateLocalSystem(
    std::vector<MatrixType>& rLeftHandSideMatrices,
...
void SmallDisplacementBbarElement::CalculateAndAddInternalForces(
...
void SmallDisplacementBbarElement::AddExplicitContribution(
...
void SmallDisplacementBbarElement::CalculateDisplacementGradient(Matrix& rH, const Matrix& rDN_DX)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateInfinitesimalStrain(const Matrix& rH, Vector& rStrainVector)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateHydrostaticDeformationMatrix(GeneralVariables& rVariables)
{
    ...
    if (dimension == 2)
    {
             ...
             PointNumber < integration_points.size(); PointNumber++)
        {
            ...
            MathUtils<double>::InvertMatrix(rVariables.J[PointNumber], InvJ,
                                            rVariables.detJ);
            noalias(rVariables.DN_DX) = prod(DN_De[PointNumber], InvJ);

            this->CalculateDeformationMatrix(rVariables.B, rVariables.DN_DX);

            // double IntegrationWeight = rVariables.detJ *
            // integration_points[PointNumber].Weight() /
            // GetGeometry().DomainSize();
            double IntegrationWeight =
                rVariables.detJ * integration_points[PointNumber].Weight();
...
double& SmallDisplacementBbarElement::CalculateTotalMass(double& rTotalMass,
                                                         const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
Vector& SmallDisplacementBbarElement::CalculateVolumeForce(Vector& rVolumeForce,
                                                           GeneralVariables& rVariables)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateFirstDerivativesContributions(
...
void SmallDisplacementBbarElement::CalculateSecondDerivativesContributions(
    MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateSecondDerivativesLHS(MatrixType& rLeftHandSideMatrix,
                                                                 ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateSecondDerivativesRHS(VectorType& rRightHandSideVector,
                                                                 ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateMassMatrix(MatrixType& rMassMatrix,
                                                       ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateDampingMatrix(MatrixType& rDampingMatrix,
                                                          ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBbarElement::CalculateOnIntegrationPoints(
...
int SmallDisplacementBbarElement::Check(const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if (VELOCITY.Key() == 0)
        KRATOS_THROW_ERROR(std::invalid_argument,
                           "VELOCITY has Key zero! (check if the application "
                           "is correctly registered",
                           "")

    if (DISPLACEMENT.Key() == 0)
        KRATOS_THROW_ERROR(std::invalid_argument,
                           "DISPLACEMENT has Key zero! (check if the "
                           "application is correctly registered",
                           "")

    if (ACCELERATION.Key() == 0)
        KRATOS_THROW_ERROR(std::invalid_argument,
                           "ACCELERATION has Key zero! (check if the "
                           "application is correctly registered",
                           "")

    if (DENSITY.Key() == 0)
        KRATOS_THROW_ERROR(std::invalid_argument,
                           "DENSITY has Key zero! (check if the application is "
    ...
    if (dimension == 2)
    {
        ...
        //     used. This is a 2D element! expected strain size is 3 (el id = )
        //     ", this->Id() ) //fails in some 2D cases, i.e. axisymmetric

        // if ( THICKNESS.Key() == 0 )
        //     KRATOS_THROW_ERROR( std::invalid_argument, "THICKNESS has Key
        //     zero! (check if the application is correctly registered", "" )
        //     //if is not read from model part it will not exist
    ...
    else
    {
        if (this->GetProperties().GetValue(CONSTITUTIVE_LAW)->GetStrainSize() != 6)
            KRATOS_THROW_ERROR(std::logic_error,
                               "wrong constitutive law used. This is a 3D "
                               "element! expected strain size is 6 (el id = ) ",
                               this->Id())
    }
