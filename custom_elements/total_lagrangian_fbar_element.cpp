...
TotalLagrangianFbarElement::TotalLagrangianFbarElement( IndexType NewId, GeometryType::Pointer pGeometry )
...
TotalLagrangianFbarElement::TotalLagrangianFbarElement( IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties )
...
Element::Pointer TotalLagrangianFbarElement::Create( IndexType NewId, NodesArrayType const& rThisNodes, PropertiesType::Pointer pProperties ) const
{
    return Element::Pointer( new TotalLagrangianFbarElement( NewId, GetGeometry().Create( rThisNodes ), pProperties ) );
}

//************************************CLONE*******************************************
//************************************************************************************

Element::Pointer TotalLagrangianFbarElement::Clone( IndexType NewId, NodesArrayType const& rThisNodes ) const
{

    TotalLagrangianFbarElement NewElement( NewId, GetGeometry().Create( rThisNodes ), pGetProperties() );
...
//************************************************************************************
    ...
    void TotalLagrangianFbarElement::CalculateAndAddLHS(LocalSystemComponents& rLocalSystem, GeneralVariables& rVariables, double& rIntegrationWeight)
    {
...
//***************************** JLM Modifico la forma en que se calcula Kgeo para FBAR
...

    {
		...
		if (dimension == 3) { 
			KRATOS_THROW_ERROR(std::invalid_argument, "something is wrong with the dimension, solo esta hecho para 2D!!!", ""); //	voigt_size = 6; 
		}
...
void TotalLagrangianFbarElement::CalculateDeformationMatrix(Matrix& rB,
        Matrix& rF,
        Matrix& rDN_DX)
{
    KRATOS_TRY
...
double& TotalLagrangianFbarElement::CalculateTotalMass( double& rTotalMass, const ProcessInfo& rCurrentProcessInfo )
{
    KRATOS_TRY
...
void TotalLagrangianFbarElement::CalculateOnIntegrationPoints( const Variable<double>& rVariable, std::vector<double>& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
...
void TotalLagrangianFbarElement::GetHistoricalVariables( GeneralVariables& rVariables, const double& rPointNumber )
{
...
double& TotalLagrangianFbarElement::CalculateVolumeChange( double& rVolumeChange, GeneralVariables& rVariables )
{
    KRATOS_TRY
