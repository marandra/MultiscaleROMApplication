...
namespace Kratos
{
    ...
    void SmallDisplacementStrElement::CalculateAll(
...
//             }
    ...
    inline void SmallDisplacementStrElement::CalculateAndAdd_ExtForceContribution(
        const Vector& N,
    ...
    void SmallDisplacementStrElement::CalculateB(
        Matrix& rB,
        const Matrix& DN_DX,
    ...
    Matrix SmallDisplacementStrElement::ComputeEquivalentF(const Vector& rStrainTensor)
    {
        const unsigned int dim = GetGeometry().WorkingSpaceDimension();
        Matrix F(dim,dim);
    ...
    int  SmallDisplacementStrElement::Check( const ProcessInfo& rCurrentProcessInfo )
    {
        KRATOS_TRY
